 ...
 */
export function validate(policyModel: PolicyModel): ValidationResult {
...
function validateSection(
  ...
  const orders = section.items.map((item: PolicyItem) => item.order).filter((o: number) => o !== undefined);
  const duplicateOrders = orders.filter((o: number, index: number) => orders.indexOf(o) !== index);
  ...
  // Validate each item
  section.items.forEach((item: PolicyItem, index: number) => {
...
function validatePolicyItem(
  ...
  switch (item.type) {
    case 'catalog':
      validateCatalogPolicyItem(path, item as CatalogPolicyItem, errors, warnings);
      break;
    case 'fragment':
      validateFragmentPolicyItem(path, item as FragmentPolicyItem, errors, warnings);
      break;
    case 'custom-xml':
      validateCustomXmlPolicyItem(path, item as CustomXmlPolicyItem, errors, warnings);
      break;
    case 'expression':
      validateCustomExpressionItem(path, item as CustomExpressionItem, errors, warnings);
      break;
    default:
      errors.push({
        path,
        message: `Unknown policy item type: ${(item as any).type}`,
        code: 'UNKNOWN_TYPE'
      });
  }
}
 ...
 */
function validateCatalogPolicyItem(
  path: string,
  item: CatalogPolicyItem,
  errors: ValidationError[],
  warnings: ValidationWarning[]
): void {
  if (!item.policyId) {
  ...
  if (item.policyId === 'send-request' && item.configuration) {
    validateSendRequestConfiguration(`${path}.configuration`, item.configuration as any, errors, warnings);
  }
}
 ...
 */
function validateFragmentPolicyItem(
  path: string,
  item: FragmentPolicyItem,
  errors: ValidationError[],
  warnings: ValidationWarning[]
): void {
 ...
 */
function validateCustomXmlPolicyItem(
  path: string,
  item: CustomXmlPolicyItem,
  errors: ValidationError[],
  warnings: ValidationWarning[]
): void {
 ...
 */
function validateCustomExpressionItem(
  path: string,
  item: CustomExpressionItem,
  errors: ValidationError[],
  warnings: ValidationWarning[]
): void {
 ...
 */
function validateSendRequestConfiguration(
  path: string,
  config: SendRequestConfiguration,
  errors: ValidationError[],
  warnings: ValidationWarning[]
): void {
  // Validate URL
 ...
 */
export function validateNamedValueReference(
  path: string,
  ref: NamedValueReference,
  errors: ValidationError[],
  warnings: ValidationWarning[]
): void {
 ...
 */
export function checkDuplicateVariableNames(policyModel: PolicyModel): ValidationWarning[] {
  const warnings: ValidationWarning[] = [];
  const variableNames = new Set<string>();

  const checkSection = (sectionName: string, section: any) => {
    ...
    section.items.forEach((item: PolicyItem, index: number) => {
      if (item.type === 'catalog' && item.configuration) {
        const catalogItem = item as CatalogPolicyItem;
