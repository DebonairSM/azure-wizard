...
export interface NamedValueReference {
  type: 'named-value';
  name: string;
  keyVault?: boolean; // true if backed by Key Vault
}
...
export interface PolicySection {
  items: PolicyItem[];
  includeBase?: boolean; // defaults to true
}
...
export type PolicyItem =
  | CatalogPolicyItem      // Built-in policy from catalog
  | FragmentPolicyItem    // Policy fragment reference
  | CustomXmlPolicyItem   // Raw XML snippet
  | CustomExpressionItem;  // C# expression
...
export interface CustomExpressionItem {
  ...
  order: number;
  targetElement?: string; // Element name this expression applies to
  targetAttribute?: string; // Attribute name if context is 'attribute'
}
...
export interface ValidationError {
  path: string; // e.g., "sections.inbound.items[0].configuration.url"
  message: string;
  code: string;
}
...
export interface PolicyDetectionResult {
  exists: boolean;
  policyXml?: string;
  policyModel?: PolicyModel; // if parsing successful
...
export interface PolicyParameter {
  ...
  items?: PolicyParameter; // for array types
  properties?: PolicyParameterSchema; // for object types
}
