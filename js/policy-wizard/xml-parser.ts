...
import { PolicyModel, PolicySection, PolicyItem, PolicyScope } from './types.js';
 ...
 */
export async function fromXml(xml: string): Promise<PolicyModel> {
  try {
    // Use xml2js for parsing
    return new Promise<PolicyModel>((resolve, reject) => {
 ...
 */
function parseSectionContent(section: any): any {
  if (!section) {
    return {};
  }
 ...
 */
function parsePolicyXml(parsed: any): PolicyModel {
 ...
 */
function parseSection(section: any): PolicySection {
 ...
 */
function normalizeXml2JsValue(value: any): any {
 ...
 */
function parsePolicyElement(elementName: string, element: any, order: number): PolicyItem | null {
 ...
 */
function parseSendRequestElement(element: any, order: number): PolicyItem {
  const config: any = {};

  // Extract attributes
  if (element.mode) config.mode = element.mode;
  if (element['response-variable-name']) config.responseVariableName = element['response-variable-name'];
  if (element.timeout) config.timeout = parseInt(element.timeout, 10);
  if (element['ignore-error']) config.ignoreErrors = element['ignore-error'] === 'true';
 ...
 */
function buildElementXml(elementName: string, element: any, attrs?: any, text?: string): string {
