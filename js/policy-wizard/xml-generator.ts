 ...
 */
export function toXml(policyModel: PolicyModel): string {
  let xml = '<policies>\n';
 ...
 */
function generateSection(sectionName: string, section: PolicySection): string {
  let xml = `  <${sectionName}>\n`;
 ...
 */
function generatePolicyItem(item: PolicyItem, indent: number): string {
  const indentStr = ' '.repeat(indent);

  switch (item.type) {
    case 'catalog':
      return generateCatalogPolicyItem(item as CatalogPolicyItem, indentStr);
    case 'fragment':
      return generateFragmentPolicyItem(item as FragmentPolicyItem, indentStr);
    case 'custom-xml':
      return generateCustomXmlPolicyItem(item as CustomXmlPolicyItem, indentStr);
    case 'expression':
      return generateCustomExpressionItem(item as CustomExpressionItem, indentStr);
    default:
      return '';
  }
}
 ...
 */
function generateCatalogPolicyItem(item: CatalogPolicyItem, indent: string): string {
  ...
  if (item.policyId === 'send-request') {
    return generateSendRequestPolicy(item.configuration as SendRequestConfiguration, indent);
  }
 ...
 */
function generateBasicPolicyXml(
  policyId: string,
...
function formatAttributeValue(value: any): string {
  if (typeof value === 'object' && value !== null && value.type === 'named-value') {
    const namedValue = value as NamedValueReference;
 ...
 */
function generateSendRequestPolicy(config: SendRequestConfiguration, indent: string): string {
 ...
 */
function formatStringOrNamedValue(value: StringOrNamedValue): string {
 ...
 */
function generateFragmentPolicyItem(item: FragmentPolicyItem, indent: string): string {
 ...
 */
function generateCustomXmlPolicyItem(item: CustomXmlPolicyItem, indent: string): string {
 ...
 */
function generateCustomExpressionItem(item: CustomExpressionItem, indent: string): string {
